    ...
    if len(pairs) > COLUMN_SIZE:
        pairs = pairs[modulo_page * COLUMN_SIZE : COLUMN_SIZE * (modulo_page + 1)] + [
            (
                ...
                EqInlineKeyboardButton(text="❌", callback_data="buttonclose"),
                EqInlineKeyboardButton(
                    text="➡️",
        ...
        pairs.append(
            [
                EqInlineKeyboardButton(text="➕ 𝓞𝔀𝓷𝓮𝓻 𝓤𝓼𝓮𝓻𝓫𝓸𝓽 ➕", url="tg://user?id=8333063872"),
            ]
        )
...
class ButtonUtils:
    ...
    async def create_button(
        ...
        if ButtonUtils.is_url(data):
            ...
            return InlineKeyboardButton(text=text, url=f"tg://user?id={int(data)}")
    ...
    def userbot_list(user_id, count, total_count):
        ...
        if count > 0:
            nav_buttons.append(
                InlineKeyboardButton(text="❮", callback_data=f"prev_ub {count}")
            )

        page_number = (count // 20) * 20
        nav_buttons.append(
            InlineKeyboardButton(text="Kembali", callback_data=f"bcpg_acc {page_number}")
        )

        if count < total_count - 1:
            nav_buttons.append(
                InlineKeyboardButton(text="❯", callback_data=f"next_ub {count}")
            )
        ...
        action_buttons = [
            [
                InlineKeyboardButton(text="Get OTP", callback_data=f"get_otp {count}"),
            ],
            [
                InlineKeyboardButton(text="Hapus User", callback_data=f"del_ubot {user_id}"),
                InlineKeyboardButton(text="Hapus Akun", callback_data=f"ub_deak {count}"),
            ],
        ]
    ...
    def account_list(start_index=0):
        ...
        buttons.append([InlineKeyboardButton(text="Tutup", callback_data="buttonclose")])
    ...
    def create_font_keyboard(font_list, get_id, current_batch):
        ...
        rows.append(
            [
                ...
                InlineKeyboardButton(text="❌", callback_data=f"close inline_font {get_id}"),
                InlineKeyboardButton(
